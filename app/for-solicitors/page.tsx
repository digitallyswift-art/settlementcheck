...
export default function ForSolicitors() {
  ...
  const inputClass = 'border border-gray-200 rounded-xl px-4 py-3 w-full text-sm focus:ring-2 focus:ring-green-600 focus:border-transparent outline-none transition'
      ...
      <main>
        ...
        <section className="bg-white pt-16 pb-12">
          <div className="max-w-4xl mx-auto px-4 text-center">
            <span className="inline-block bg-green-50 text-green-800 text-xs font-semibold px-4 py-1.5 rounded-full uppercase tracking-wide mb-5">
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <div key={card.title} className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
        ...
        <section id="contact" className="bg-white py-16">
          <div className="max-w-xl mx-auto px-4">
            <h2 className="text-3xl font-bold text-gray-900 text-center mb-2 tracking-tight">Apply to join our panel</h2>
            <p className="text-gray-500 text-center mb-8">SRA-regulated employment law firms only.</p>
            ...
            {submitted ? (
              <div className="text-center py-8 bg-green-50 rounded-2xl border border-green-100">
                ...
                <h3 className="font-bold text-gray-900 text-lg mb-1">Application received</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Geographic coverage</label>
                  <input type="text" className={inputClass} placeholder="e.g. London, South East" value={form.coverage}
                    onChange={(e) => setForm({ ...form, coverage: e.target.value })} />
                </div>
                <div>
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" className="mt-0.5 w-4 h-4 accent-green-700" required checked={form.sra}
                      onChange={(e) => setForm({ ...form, sra: e.target.checked })} />
                    <span className="text-sm text-gray-600">Regulated by the SRA</span>
                  </label>
                </div>
                <button type="submit" disabled={loading}
                  className="w-full bg-green-800 hover:bg-green-700 text-white font-bold py-4 rounded-xl transition-colors disabled:opacity-60">
