  ...
  return (
    <div className="rounded-lg p-7 md:p-8 bg-sage-tint" style={{ border: '1px solid #BCD0BF' }}>
      ...
      <div className="sc-body mt-3 max-w-[66ch] flex flex-col gap-3">
        ...
        <p>A strong financial figure is not the full picture. Settlement agreements include terms beyond the number — reference wording, confidentiality obligations, claims you are waiving — that can be as significant as the payment itself.</p>
...
function BreakdownTable({ result, offer, salary }: { result: VerdictResult; offer: number; salary: number }) {
  ...
  const pilonLabel = result.noticeIsContractual
    ? `Notice pay — ${result.noticeWeeksUsed} weeks PILON`
    : `Notice pay — ${result.noticeWeeksUsed} week${result.noticeWeeksUsed !== 1 ? 's' : ''} PILON`
...
function SaveCard({ resultRef, onEmailCapture }: { resultRef: React.RefObject<HTMLDivElement | null>; onEmailCapture: (email: string) => void }) {
  ...
  async function sendEmail(e: React.FormEvent) {
    e.preventDefault()
    // Stub — real implementation POSTs to /api/save-results
    setSent(true)
  ...
  return (
    <div ref={resultRef} className="bg-card border border-rule rounded-lg p-6 flex flex-col gap-4">
      ...
      <div className="flex gap-3 flex-col sm:flex-row">
        ...
        <button
          type="button"
          onClick={() => { /* PDF generation — implemented with jsPDF in full build */ alert('PDF download coming shortly.') }}
        ...
        {/* Email my results */}
        {!emailOpen && (
          <button type="button" onClick={() => setEmailOpen(true)} className="btn-ghost text-[14px] px-4 py-2.5">
            Email me my results
          </button>
        )}
      </div>
      ...
      {emailOpen && !sent && (
        <form onSubmit={sendEmail} className="flex flex-col gap-3 pt-2">
          ...
          <button type="submit" className="btn-ghost text-[14px] self-start px-4 py-2.5">Send my results</button>
        </form>
      )}
      {sent && (
  ...
  return (
    <div className="bg-card border border-rule rounded-lg p-6 md:p-7 flex flex-col gap-5">
      <div>
        ...
        <p className="sc-body mt-2 max-w-[56ch]">
          We match you to a solicitor who handles settlement agreements exclusively — not a general practice firm taking employment cases alongside conveyancing and wills — within 24 hours.
        </p>
        <p className="sc-body mt-2 max-w-[56ch]">
...
function ResultsContent() {
  ...
  const contractualNotice   = parseInt(searchParams.get('contractualNotice') ?? '0')
    ...
    <>
      ...
      <main className="bg-paper-2 min-h-screen pb-24" style={{ animation: 'sc-fade-in 400ms ease forwards' }}>
        <div className="sc-container pt-12" style={{ maxWidth: 860 }}>
          ...
          <Link href="/" className="inline-flex items-center gap-1.5 text-[14px] text-muted hover:text-ink transition-colors mb-8">
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M10 13L5 8L10 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            Adjust calculator
          </Link>
          ...
          {result && (
            <div className="flex flex-col gap-6">
              ...
              {/* 8. Save and share */}
              <SaveCard resultRef={saveCardRef} onEmailCapture={email => setPrefillEmail(email)} />

              {/* 9. Lead capture — Step 2 */}
